...
### Running the App
- **Local**: `just run` (or `uvicorn app.main:app --reload`)
- **Docker**: `just docker-up` (or `docker-compose up`)
- The API will be available at `http://localhost:8000`.
- Swagger UI: `http://localhost:8000/docs`.

### Database Migrations
- Run `just migrate` to apply alembic migrations.

### MCP Server & Agent
- Run MCP Server: `just mcp`
- Run Agent: `just agent` (Requires `GEMINI_API_KEY` in `.env`)

### Testing
- Run `just test` (or `pytest`).
- Testing approach uses `pytest`. For integration tests, a test database container should be used.
...
- [x] Test coverage ≥ 70% (achieved: **94%**). **(10 points)**
